...
class RoleController extends BaseAdminController
{
    ...
    public function indexAction(Request $request, RoleService $roleService, Grid $grid){
        ...
        $grid->setTableColumn("#", "string", "id","a.id");
        $grid->setTableColumn("名称", "string", "name");
        $grid->setTableColumn("创建时间", "datetime", "createdAt", "a.createdAt");
        $grid->setTableColumn("是否锁定", "boole", "isLock");
        $grid->setTableColumn("描述", "text", "descr");
        $grid->setSearchField("名称", "text", "name");
        $grid->setGridBar("添加", $this->generateUrl("admin_role_add"));
        $grid->setTableAction(function($obj){
            if($obj->getIsLock()) return ;
            $id = $obj->getId();
            $url = $this->generateUrl('admin_role_edit',['id'=>$id]);
            $str = '<a href='.$url.' title="编辑角色" class=" btn btn-info btn-xs poppage"><i class="fas fa-edit"></i></a>';
            return  $str;
        });
        $grid->setTableAction(function($obj){
            if($obj->getIsLock()) return ;
