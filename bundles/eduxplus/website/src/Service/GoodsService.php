...
class GoodsService extends AppBaseService
{
    ...
    public function getRecentlyLiveCourse($limit=2){
        ...
        $result = $this->db()->fetchAll($sql, [], 0, 200);
        ...
        foreach ($result as $v){
            ...
            ";
            $planResult = $this->db()->fetchOne($planSql);
            ...
            $productResult = $this->db()->fetchOne($sqlProduct);
            ...
            $goodsResult = $this->db()->fetchOne($goodsSql);
    ...
    public function getChapter($courseId, $parentId)
    {
        ...
        $list = $this->db()->fetchAll($sql, ["courseId" => $courseId, "parentId" => $parentId]);
        if (!$list) [];
        $rs = [];
        foreach ($list as &$cv) {
            $pid = $cv['parentId'];
    ...
    public function getByIds($ids){
        ...
        $infos = $this->db()->fetchAll($sql, ['id' => $ids]);
    ...
    public function getSimpleByUuid($uuid){
        ...
        $info = $this->db()->fetchOne($sql, ['uuid' => $uuid]);
        if($info){
    ...
    public function getTeachersByIds($ids)
    {
        ...
        return $this->db()->fetchAll($sql, ['id' => $ids]);
    }
    ...
    public function getGoodsGroup($id)
    {
        ...
        if (!$allGoodIds) {
            ...
            $goodsInfo =  $this->db()->fetchAll($sql4, $params);
            if ($goodsInfo) {
    ...
    public function getFav($goodsId, $uid){
        ...
        return $this->db()->fetchOne($sql, ["uid"=>$uid, "goodsId"=>$goodsId]);
    }
    ...
    public function delFav($goodsId, $uid){
        $sql = "SELECT a FROM Edux:MallGoodsFav a WHERE a.uid=:uid AND a.goodsId =:goodsId ";
        $model =  $this->db()->fetchOne($sql, ["uid"=>$uid, "goodsId"=>$goodsId], 1);
    ...
    public function favList($uid, $page, $pageSize){
        ...
        if($items) {
            foreach ($items as $v) {
                $vArr = $this->toArray($v);
                $sql = "SELECT a FROM Edux:MallGoods a WHERE a.id =:id ";
                $goodsInfo = $this->db()->fetchOne($sql, ["id"=>$vArr['goodsId']]);
