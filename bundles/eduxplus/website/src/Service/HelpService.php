...
class HelpService extends AppBaseService
{
    ...
    public function getCategoryAndHelp($parentId=0){
        $sql = "SELECT a FROM Cms:CmsHelpCategory a WHERE a.parentId =:parentId AND a.isShow=1  ORDER BY a.sort ASC";
        ...
        foreach ($list as &$v) {
            ...
            //如果没有子分类
            if(($parentId == 0) && (!$child)){
                $sql = "SELECT a FROM Cms:CmsHelp a WHERE a.categoryId =:categoryId AND a.status=1 ORDER BY a.createdAt ASC";
    ...
    public function getById($id){
        $sql = "SELECT a FROM Cms:CmsHelp a WHERE a.id=:id AND a.status=1";
        $detail = $this->db()->fetchOne($sql, ["id"=>$id]);
        if(!$detail) return $detail;
        $mainSql = "SELECT a FROM Cms:CmsHelpMain a WHERE a.helpId =:helpId ";
    ...
    public function getCategoryById($id){
        $sql = "SELECT a FROM Cms:CmsHelpCategory a WHERE a.id =:id AND a.isShow=1 ";
        $list = $this->db()->fetchOne($sql, ["id"=>$id]);
        return $list;
    }

    public function getAllTopValueHelps(){
        $sql = "SELECT a FROM Cms:CmsHelp a WHERE a.topValue >0 AND a.status=1 ORDER BY a.sort ASC";
