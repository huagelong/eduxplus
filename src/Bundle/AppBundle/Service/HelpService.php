...
class HelpService extends AppBaseService
{
    ...
    public function getCategoryAndHelp($parentId=0){
        $sql = "SELECT a FROM App:MallHelpCategory a WHERE a.parentId =:parentId AND a.isShow=1  ORDER BY a.sort ASC";
        ...
        foreach ($list as &$v) {
            ...
            //如果没有子分类
            if(($parentId == 0) && (!$child)){
                $sql = "SELECT a FROM App:MallHelp a WHERE a.categoryId =:categoryId AND a.status=1 ORDER BY a.createdAt ASC";
    ...
    public function getById($id){
        $sql = "SELECT a FROM App:MallHelp a WHERE a.id=:id AND a.status=1";
        $detail = $this->fetchOne($sql, ["id"=>$id]);
        if(!$detail) return $detail;
        $mainSql = "SELECT a FROM App:MallHelpMain a WHERE a.helpId =:helpId ";
    ...
    public function getCategoryById($id){
        $sql = "SELECT a FROM App:MallHelpCategory a WHERE a.id =:id AND a.isShow=1 ";
        $list = $this->fetchOne($sql, ["id"=>$id]);
        return $list;
    }

    public function getAllTopValueHelps(){
        $sql = "SELECT a FROM App:MallHelp a WHERE a.topValue >0 AND a.status=1 ORDER BY a.sort ASC";
